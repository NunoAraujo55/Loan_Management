...
class _HomeScreenState extends State<HomeScreen> {
    ...
    try {
      ...
      for (final loan in loans) {
        ...
        if (variablePeriod.values.isNotEmpty) {
          ...
          final int periodicidade = rates.first.term ?? 0;
        ...
        loanPeriodicities[loan.id!] =
            activePeriod.values.isNotEmpty ? (activePeriod.values.first.term ?? 0) : 0;
  ...
  List<AmortEntry> simulateLoan({
    ...
    while (balance > 0.01 && month <= totalMonths) {
      ...
      /*if (printed < 70) {
        print("month: $month | payment: $pmt | rate: $annualRate");
        printed++;
      }*/
